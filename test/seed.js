...
describe("Contract: Seed", async () => {
  ...
  const CLASS_PERSONAL_FUNDING_LIMIT = ethers.BigNumber.from(
    "100000000000000000000"
  );
  const CLASS_SMALL_PERSONAL_FUNDING_LIMIT = ethers.BigNumber.from(
    "20000000000000000000"
  );
  const CLASS_18_PERSONAL_FUNDING_LIMIT =
    ethers.BigNumber.from("180000000000000000").toString(); // = 2 * smallBuyAmount
  const CLASS_20_PERSONAL_FUNDING_LIMIT =
    ethers.BigNumber.from("200000000000000000").toString();
  ...
  context("» creator is avatar", () => {
    before("!! setup", async () => {
      ...
      CLASS_VESTING_START_TIME = (
        await startTime.add(await time.duration.years(10))
      ).toNumber();
    ...
    context("» contract is not initialized yet", () => {
      context("» parameters are valid", () => {
        ...
        it("it initializes seed", async () => {
          ...
          let startTime = (await time.latest()).add(
            await time.duration.minutes(1)
          );
      ...
      context("» generics", () => {
        ...
        it("cannot buy more than personal maximum allows", async () => {
          ...
          await alternativeSetup.seed
            ...
            .addClass(
              hardCap,
              CLASS_SMALL_PERSONAL_FUNDING_LIMIT,
              price,
              CLASS_VESTING_DURATION,
              CLASS_VESTING_START_TIME,
              CLASS_FEE
          ...
          await alternativeSetup.seed
            .connect(buyer1)
            .buy(CLASS_SMALL_PERSONAL_FUNDING_LIMIT);
        ...
        it("it returns amount of seed token bought and the fee", async () => {
          ...
          expect((await seedAmount).toString()).to.equal(
            buySeedAmount.toString()
          );
        ...
        it("cannot buy more than maximum target", async () => {
          ...
          await alternativeSetup.seed.addClass(
            hardCap,
            hardCap,
            price,
            CLASS_VESTING_DURATION,
          ...
          await alternativeSetup.seed
            .connect(admin)
            .setClassBatch([buyer1.address, buyer3.address], [0, 2]);

          await alternativeSetup.seed.setClass(buyer1.address, 1);

          time.increase(await time.duration.hours(1));

          await alternativeSetup.seed.connect(buyer1).buy(softCap);

          await expectRevert(
            alternativeSetup.seed.connect(buyer3).buy(hardCap),
        ...
        it("vestingStartTime == current timestamp", async () => {
          ...
          const expectedClaim = (await time.latest()).add(
            new BN(timeDifference)
          );
        ...
        context("» ERC20 transfer fails", () => {
          it("reverts 'Seed: funding token transferFrom Failed' ", async () => {
            ...
            startTime = (await time.latest()).add(
              await time.duration.minutes(1)
            );
            await alternativeSetup.seed.initialize(
              beneficiary.address,
    ...
    context("# claim", () => {
      context("» softCap not reached", () => {
        ...
        beforeEach(async () => {
          ...
          startTime = (await time.latest()).add(await time.duration.minutes(1));
          await alternativeSetup.seed.initialize(
            beneficiary.address,
            admin.address,
            [seedToken.address, fundingToken.address],
      ...
      context("» claim after vesting duration", async () => {
        before("!! deploy new contract + top up buyer balance", async () => {
          ...
          await setup.data.seed.connect(admin).setClass(buyer1.address, 3);
        });
        ...
        it("it cannot claim before currentVestingStartTime", async () => {
          ...
          await expectRevert(
            setup.data.seed
              ...
              .claim(
                buyer1.address,
                new BN(softCap).mul(new BN(twoBN)).toString()
              ),
      ...
      context("» claim when vesting duration is 0", async () => {
        ...
        it("claims all seeds after vesting duration", async () => {
          ...
          const claimTemp = new BN(buySeedAmount)
            .mul(new BN(twoBN))
            .div(new BN(divisor))
            .toString();
        ...
        it("it claims all the fee", async () => {
          ...
          expect(dividedFeeAmountRequired.toString()).to.equal(
            feeClaimed.toString()
          );
        });
        it("funds DAO with all the fee", async () => {
          ...
          const dividedFeeAmountRequired = ethers.BigNumber.from(
            (fee / divisor).toString()
          );
      ...
      context("» ERC20 transfer fails", () => {
        it("reverts 'Seed: seed token transfer Failed' ", async () => {
          ...
          await fakeSeedToken
            .connect(root)
            .mint(alternativeSetup.seed.address, requiredSeedAmount.toString());
    ...
    context("# close", () => {
      ...
      context("» close after minimum reached", () => {
        ...
        it("it reverts setClass when Seed is closed", async () => {
          // await setup.data.seed.connect(admin).close();
          await expectRevert(
            setup.data.seed.connect(admin).setClass(buyer3.address, 0),
            "Seed: should not be closed"
          );
        });
      });
    ...
    context("# admin functions", () => {
      before("!! setup", async () => {
        ...
        await fundingToken
          .connect(buyer2)
          .approve(setup.seed.address, smallBuyAmount);
      ...
      context("» add class", () => {
        it("it adds class", () => {
          context("» generics", () => {
            before("!! setup", async () => {
              ...
              setup.seed = await init.getContractInstance(
                "Seed",
                setup.roles.prime
              );
            ...
            it("it adds Customer class", async () => {
              ...
              expect((await setup.seed.classes(0))[0]).to.equal(
                ethers.BigNumber.from(hardCap)
              );
            });
            ...
            it("it adds Customer class", async () => {
              await setup.seed
                ...
                .addClassBatch(
                  ...
                  [e_twenty, 1e6],
                  [10000000, 10000],
                  [CLASS_VESTING_START_TIME, CLASS_VESTING_START_TIME],
                  [CLASS_FEE, CLASS_FEE]
                );
              expect((await setup.seed.classes(2))[1]).to.equal(
                ethers.BigNumber.from(e_twenty)
              );
            });
            it("it reverts when fee >= 45% for Customer class", async () => {
              const feeTooBig = parseEther("0.45").toString(); // 45%
        ...
        it("it reverts when trying to add > 100 classes", () => {
          context("» generics", () => {
            it("it adds Customer class", async () => {
              ...
              const cvdArr = Array.from(
                Array(101).fill(CLASS_VESTING_DURATION)
              );
              const cvstArr = Array.from(
                Array(101).fill(CLASS_VESTING_START_TIME)
              );
        ...
        it("it reverts when trying to set non existent class with setClass", async () => {
          await expectRevert(
            setup.seed.connect(admin).setClass(buyer3.address, 101),
      ...
      context("» change class", () => {
        ...
        it("it changes Customer class", async () => {
          ...
          expect((await setup.data.seed.classes(0))[0]).to.equal(
            ethers.BigNumber.from(e_twenty)
          );
        });
      ...
      context("» change class", () => {
        before("!! deploy new contract", async () => {
          ...
          let newClassVestingStartTime = await newEndTime.add(
            await time.duration.days(1)
          );
          ...
          await setup.data.seed
            ...
            .addClass(
              ...
              price,
              CLASS_VESTING_DURATION,
              newClassVestingStartTime.toNumber(),
              CLASS_FEE
            );
        });
        it("it reverts when trying to set class when vesting is already started", async () => {
          await time.increase(time.duration.days(2));
          await expectRevert(
            setup.seed.connect(admin).setClass(buyer2.address, 0),
            "Seed: vesting is already started"
          );
        });
      });
  ...
  context(
    "creator is avatar -- whitelisted contract few classes simultanuosly test",
    () => {
      before("!! deploy setup", async () => {
        setup = await deploy();
        ...
        newClassVestingStartTime = await endTime.add(
          await time.duration.days(4)
        );
        localVestingDuration = time.duration.days(10);
        localVestingCliff = time.duration.days(5);
        SECOND_CLASS_VESTING_START_TIME = await newClassVestingStartTime.add(
          await time.duration.days(2)
        );
        CLASS_TWO_PERSONAL_FUNDING_LIMIT = ethers.BigNumber.from(
          "18000000000000000000"
        ).toString();
        CLASS_25_PERSONAL_FUNDING_LIMIT =
          ethers.BigNumber.from("250000000000000000").toString();
      });
      context(
        "# few classes simultanuosly; contract whitelisted version",
        () => {
          it("initializes", async () => {
            // emulate creation & initialization via seedfactory & fund with seedTokens
            startTime = (await time.latest()).add(await time.duration.days(1));
          ...
          it("adds new classes", async () => {
            await setup.seed //class 1 and 2
              ...
              .addClassBatch(
                [
                  CLASS_SMALL_PERSONAL_FUNDING_LIMIT,
                  CLASS_20_PERSONAL_FUNDING_LIMIT,
                ],
                [
                  CLASS_18_PERSONAL_FUNDING_LIMIT,
                  CLASS_18_PERSONAL_FUNDING_LIMIT,
                ],
                [price, price],
                [
                ...
                [
                  newClassVestingStartTime.toNumber(),
                  SECOND_CLASS_VESTING_START_TIME.toNumber(),
                ],
          ...
          it("it changes Customer class 1", async () => {
            ...
            expect((await setup.seed.classes(1))[1]).to.equal(
              ethers.BigNumber.from(CLASS_20_PERSONAL_FUNDING_LIMIT)
            );
            expect((await setup.seed.classes(1))[2]).to.equal(
              ethers.BigNumber.from(price_class1)
            );
          });
          it("it whitelists users", async () => {
            await setup.seed.connect(admin).whitelist(buyer1.address, 2);
            await setup.seed.connect(admin).whitelist(buyer3.address, 1);
            await setup.seed.connect(admin).whitelist(buyer4.address, 1);
          ...
          it("it changes class", async () => {
            await setup.seed.connect(admin).setClass(buyer4.address, 2);
            expect(
              (await setup.seed.funders(buyer4.address))[0].toString()
            ).to.equal(ethers.BigNumber.from(2).toString());
          });
          context("» unwhitelist", () => {
            it("removes a user from the whitelist", async () => {
              expect(await setup.seed.whitelisted(buyer4.address)).to.equal(
                true
              );
            ...
            it("reverts when unwhitelist account buys", async () => {
              await expectRevert(
                setup.seed
                  .connect(buyer4)
                  .buy(getFundingAmounts("1").toString()),
                "Seed: sender has no rights"
              );
            });
          });
          ...
          it("it buys tokens buyer1", async () => {
            ...
            expect(
              ...
              Math.floor(
                ((buySmallSeedAmount * price) / PRECISION) * 2
              ).toString() // * 2 because we already bought from buyer3
            );
          });
          ...
          it("claims all seeds after vesting duration buyer3", async () => {
            ...
            setup.prevBalance = await seedToken.balanceOf(beneficiary.address);

            smallBuyAmount = getFundingAmounts("9").toString();
            // amountClaimable 1020000000 --> 10200000000000000/1020000000 = 1000000000
            const divisor = 1000000000;
            const claimTemp = new BN(smallBuyAmount)
              .mul(new BN(twoBN))
              .div(new BN(divisor))
              .div(new BN(2))
              .toString();
          ...
          it("updates the amount of funding token withdrawn", async () => {
            const maxWithdrawAmount = ethers.BigNumber.from(
              getFundingAmounts("9").mul(2)
            );
            await expect(
  ...
  context(
    "creator is avatar -- non-whitelisted contract few classes simultanuosly test",
    () => {
      before("!! deploy setup", async () => {
        setup = await deploy();
      ...
      context(
        "# few classes simultanuosly non-whitelisted version new context",
        () => {
          it("initializes", async () => {
            // emulate creation & initialization via seedfactory & fund with seedTokens
            permissionedSeed = false;
          ...
          it("it sets class", async () => {
            await setup.seed.connect(admin).setClass(buyer1.address, 1);
            await setup.seed.connect(admin).setClass(buyer3.address, 1);
          ...
          it("it changes class", async () => {
            await setup.seed.connect(admin).setClass(buyer1.address, 2);
            expect(
              (await setup.seed.funders(buyer1.address))[0].toString()
            ).to.equal(ethers.BigNumber.from(2).toString());
          });
